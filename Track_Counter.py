#python3 Track_Counter.py --source 0  --dwell-time --nosave --classes 0  --save-txt --count --sendData
...
# api-endpoint
API_ENDPOINT = "http://rewamp.experiences.digital/api/peoplecount?realtime=1"

def detect(save_img=False, line_thickness=1):

    open("data.json", "w").close()

    global dtime, dwellx, avg_dwellTime, p_count, spf, prev_frame_time, new_frame_time, prev_now
    prev_now = 0
    new_frame_time = 0
    prev_frame_time = 0
    p_count = 0
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if dwell_time:
                            # object trajectory
                            #vid_capx = cv2.VideoCapture(2)
                            #fpsx = fps = vid_capx.get(cv2.CAP_PROP_FPS)  
                            ...
                            cv2.putText(im0, str((int(dwellx[id]))), center, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)                            
                    ...
                    #timestamp, devid, avdwt, pc
                    # defining a params dict for the parameters to be sent to the API
                    
                       
                           
                    #print("People Count: ", max(id))
                    try:
            ...
            if count:
                ...
                try:
                    ...
                    p_count = itemDict.get('person')
                    #print("People Count", p_count)
                ...
                if sendData:
                    ...
                    data = {"devID": int(devID), "timeStamp": str(date_time), "p_count": int(p_count), "avg_dwellTime": int(avg_dwellTime)}
            ...
            spf = new_frame_time-prev_frame_time
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dwell-time', action='store_true', help='display object dwell time and calculate average dwell time')
    parser.add_argument('--sendData', action='store_true', help='send data to cloud')
